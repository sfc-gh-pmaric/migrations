...
parser.add_argument('-future', '--future',type=str,
    help='create future grants for specified databases in dbfile ')
...
parser.add_argument('-parallel','--parallel', type=int, default=1, 
    help='Enter number of threads to use for running DR scripts in DR mode')

parser.add_argument('-scriptslistfile', '--scriptslistfile',  type=str, help='scripts to run')

parser.add_argument('-splitbyschemafile', '--splitbyschemafile',  type=str,
    help='a file name with list of sql files that need to be split by schema')

parser.add_argument('-splitbylinesfile', '--splitbylinesfile',  type=str,
    help='a file name with list of sql files that need to be split by number of lines')

parser.add_argument('-numberoflines', '--numberoflines',  type=int, default=25000, help='number of lines to split by')
...
#db = args.priv
dbfile = args.dbfile
splitfiles = args.splitbyschemafile 
splitfilesbylines = args.splitbylinesfile 
scriptslistfile = args.scriptslistfile 
...
if crossrep.mode == 'DR' or crossrep.mode == 'DR_TEST':
    ...
    if len(split_files_list)==0:  
        if splitfilesbylines:
            split_files_list = crossrep.readFile(migHome + splitfilesbylines)
            if len(split_files_list) > 0: split_method = 'lines'
        else:
            split_files_list = [] 
    print("before printing split files list")
    print(split_files_list)
    if scriptslistfile:
        scripts_list = crossrep.readFile(migHome + scriptslistfile)
    else:
        scripts_list = [] 
    print(scripts_list)

    filelist  = scriptloader.list_scripts(migHome, "scripts", scripts_list)
    print("before printing filelist")
    print(filelist)
    run_time = datetime.now()
    #l_cur = con.cursor()
    #l_cur.execute('ALTER SESSION SET CLIENT_SESSION_KEEP_ALIVE=TRUE') # keep session alive as workers can take a long time 
    run_dir = os.path.join(migHome,"script_runs", run_time.strftime("%Y%m%d"))
    if not os.path.exists(run_dir):
        os.makedirs(run_dir)

    scriptloader.copy_files_to_directory(filelist, run_dir)

    tmp_dir = os.path.join(migHome,"tmp")
    if not os.path.exists(tmp_dir):
        os.makedirs(tmp_dir)

    log_dir = os.path.join(migHome,"logs", run_time.strftime("%Y%m%d"))
    ...
    status_object = scriptloader.upload_multithreaded(
        threads,crossrep.mode,ctx,creds,filelist,tmp_dir,log_dir,crossrep.verbose,
        continue_on_error=True, option=ddl_option, split_files_list=split_files_list, 
        failed_queries_log_file=crossrep.getEnv('FAILED_QUERIES_LOG_FILE'), split_method=split_method) 
...
if args.create:
    ...
    try:
        ...
        elif coption == '4' or coption == '5': # Renga's change 
            #crossrep.crParent( cursor) 
            crossrep.crParentandPrivilegesAU(cursor) # Renga's change
        #elif coption == '5':
            #crossrep.crPriv( cursor)
            #crossrep.crPriv_from_acctusage(cursor) # create table grants_to_roles as select * from snowflake.account_usage.grants_to_roles          
        elif coption == '6':    
...
if dbfile :
    ...
    # generate grants for warehouses separately 
    gwf = open(migHome + "scripts/rbac/28_grant_warehouse_privileges.sql","w")
    crossrep.grantByObjType('WAREHOUSE', gwf, cursor)
...
## -dis option: discover how many objects in the account 
if args.discover:
    ofile = open(migHome + "scripts/eval/a0_disable_users.sql","w")
    crossrep.countOBJ( ofile, cursor)
    ofile.close()
...
if args.future:
    ...
    try:
        ...
        future_dblist = crossrep.readFile(dbfilepath)
        crossrep.crFGrant(cursor, future_dblist)
...
if args.monitor : 
    ...
    if crossrep.mode == 'CUSTOMER':
        ...
        cursor = tgt_ctx.cursor()
        cursor.execute("USE DATABASE "+tdb)
        cursor.execute("USE SCHEMA "+tsc)
    '''
    ...
    rec = cursor.fetchall()
    rf.write('\t DBNAME \t TOTAL_COPY_TBBYTES \t COPY_RATE_TB_BY_HOUR \n')
    for r in rec:
        rf.write('\t'+r[0] + '    \t'+str(r[1]) + '\t'+str(r[2])+'\n')
    '''
...
if args.test:
    ...
    # excluding: ADMIN, KHOYLE, MYANG, DBRYANT, MBROWN from dropping list
...
if args.validate:
    ...
    try:
        ...
        if crossrep.mode == 'CUSTOMER':
            ...
            '''
            #rfile = open(migHome + "scripts/eval/c1_rowcount.txt"+ver+".sql","w")
            #hfile = open(migHome + "scripts/eval/c2_hashagg_"+ver+".sql","w")
            #crossrep.valCount( rfile, cursor, tgt_cursor)
            #crossrep.valHash( hfile, cursor, tgt_cursor)
            #crossrep.valCountHash(val_dblist, rfile, cursor, tgt_cursor)
