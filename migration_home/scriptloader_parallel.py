...
def list_scripts(migHome, objectType='', filelist=[]):
    ...
    for root, dirs, files in os.walk(root_folder):
        ...
        for file in files:
            #print(file)
            if file.endswith(".sql") and (len(filelist) == 0 or (file in filelist)): 
                # sort_on = "key2"
                l_dict = {}
                l_dict['file'] = file
                l_dict['path'] = root 
    ...
    if len(filelist) > 0:
        sorted_list = sorted(l_list, key=lambda x: filelist.index(x['file']), reverse=False)
    else: 
        sorted_list = sorted(l_list, key=lambda i: i['file'], reverse=False)
    return sorted_list 

def copy_files_to_directory(filelist, target_dir):
    for f in filelist:
        shutil.copyfile(os.path.join(f['path'], f['file']), os.path.join(target_dir, f['file'])) 

def split_files_by_lines(dir, target_dir, split_files_list, out_remove_files, out_add_files, threads=1):
    for file_name in split_files_list:
        file_name_wo_ext = file_name[0:file_name.find('.')]
        file_contents = ''
        file_content_array = []
        ...
        dellist = glob.glob(os.path.join(target_dir, file_name_wo_ext + "_*.sql"))
        ...
        with open(os.path.join(dir, file_name), "r") as f:
            k = 0
            lines = f.readlines() 
            num_lines = math.ceil(len(lines)/threads)
            #print(file_name)
            #print(num_lines)
            f.seek(0,0)
            for ctr, line in enumerate(f):
                #print(ctr)
                if ctr < (k+1)*num_lines:
                    s = s + line 
                else:
                    # print(f"file: {k}, length: {len(s)}")
                    # write it out
                    s = s + line 
                    new_file_name = os.path.join(target_dir,file_name_wo_ext+f"_{str(k)}.sql")
                    # print(new_file_name)
                    with open(new_file_name, "w") as tf:
                        tf.write(s)
                        out_add_files['remainder'].append(new_file_name) 
            ...
            if len(s) > 0:
                ...
                s = '' 
            # print(s)
            out_remove_files.append(os.path.join(dir, file_name))
...
def record_status(
    ...
    if return_type == 'error':
        e = exception_record 
        status_dict = {}
        status_dict['return_message'] = e.msg if hasattr(e,'msg') else ''
        status_dict['query_id'] = e.sfqid if hasattr(e,'sfqid') else '' 
        status_dict['rowcount'] = 0
        status_dict['sqlstate'] = e.sqlstate if hasattr(e, 'sqlstate') else '' 
        status_dict['errorcode'] = e.errno if hasattr(e, 'errno') else '' 
        status_dict['messages'] = e.msg if hasattr(e, 'msg') else '' 
        status_dict['filename'] = filename
        status_dict['captured_ts'] =  datetime.now(tz=pytz.utc).strftime("%Y-%m-%d %H:%M:%S")
...
def cleanse_files(sqlfiles, tmp_dir, tmp_sqlfiles, option):
    for sqlfile in sqlfiles: 
        ...
        with open(os.path.join(sqlfile['path'], sqlfile['file']), "r", encoding='utf-8', errors='replace') as f:
            s = f.read()
            print(len(s)) 
            s0 = re.sub(r'^\s*;',r';',s,flags=re.MULTILINE | re.IGNORECASE)
            s1 = re.sub(r'resource_monitor\s*=\s*null', '', s0, flags=re.MULTILINE | re.IGNORECASE)
            s2_arr = []
            ...
            s2 = '\n'.join(s2_arr)
            #print(s1)
            if option == USE_CREATE_OR_REPLACE and (not s2.strip().upper().startswith("GRANT")):
                s3 = use_create_or_replace_object(s2)
            elif (not s2.strip().upper().startswith("GRANT")): 
                # create or replace <object>
                s3 = use_create_object_if_not_exists(s2)
            else:
                s3 = s2 
            with open(tmp_sqlfile,"w", encoding="utf-8") as outf:
                outf.write(s3)
...
"""
def upload_multithreaded(thread_count, mode, con, creds, sqlfiles, tmp_dir, log_dir, verbose, 
    continue_on_error=False, option=USE_CREATE_IF_NOT_EXISTS, split_files_list=[],
    failed_queries_log_file='failed_queries.log', split_method='lines'):
    # upload_multithreaded function runs scripts in multithreaded fashion . 
    # calling function needes to pass in files that can be run in parallel 
    status_object = {'error_counter':0, 'error_exists': 'N', 'failed_statements':[], 'other_statements':[]}
    l_failed_sql_file = failed_queries_log_file if failed_queries_log_file else 'failed_queries.log'
    failed_sql_filename = os.path.join(log_dir, l_failed_sql_file)
    ...
    if len(split_files_list) > 0 and split_method=='schema':
        split_files_by_schema(tmp_dir, split_files_list, out_remove_files, out_add_files)
    if len(split_files_list) > 0 and split_method=='lines':
        print("before split_files_by_list call")
        split_files_by_lines(tmp_dir, tmp_dir, split_files_list, out_remove_files, out_add_files,thread_count)
    # remove files from tmp_sqlfiles 
    #print(len(tmp_sqlfiles))
    print("before printing out_add_files")
    print(out_add_files)
    ...
    print("before printing tmp_sqlfiles")
    print(len(tmp_sqlfiles))
    print(tmp_sqlfiles)
    ...
    for session_id in session_id_arr:
        get_failed_statements(retry_con, session_id , t0.strftime("%Y-%m-%d %H:%M:%S"), datetime.now(tz=pytz.utc).strftime("%Y-%m-%d %H:%M:%S"), retry_list) 
    ...
    with open(failed_sql_filename+'.before_retry', "w") as ff:
        for sq in retry_list:
            ...
            ff.write('\n')
            ff.write('*/ \n')
    logger.info("finished logging failed_statements into <sql_filename>.before_retry file") 
    final_failed_list = retry_list # as we are not going to retry the failed queries
    """
