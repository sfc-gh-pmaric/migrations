...
#    31_create_stages_DDL.sql   => DDL to create stages
#    32_create_pipes_DDL.sql    => DDL to create pipes 
...
### generating a database's DDL for the database and all objects in the database
...
def genAllStageDDL( ofile, cursor):
    ...
    for r in rec:
        ...
        if crossrep.isBlank (stage_url ) == False :
            crSQL = crSQL + " URL =  '" + stage_url + "' comment='" + (comment or '') + "'"
            if crossrep.verbose==True:
                print(crSQL)
        ofile.write(crSQL+';\n')
