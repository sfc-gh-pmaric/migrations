...
def crUsers( cursor):
    ...
    cquery = ( "create or replace temp table " + tb_temp +
        ...
        "from table(result_scan(last_query_id())) where \"disabled\" = false")
...
def crParent( cursor):
    ...
    cursor.execute("begin")
    cursor.execute("show grants of role " + crossrep.acctpref_qualifier + "ACCOUNTADMIN")
    cursor.execute("create or replace table " + tbname + " as select distinct $1 created, $2 role_name, $3 granted_to , $4 grantee_name, $5 granted_by from table(result_scan(last_query_id()))")
    cursor.execute("commit")

    query = "select distinct role_name from " + crossrep.tb_role + " where role_name != 'ACCOUNTADMIN' "
    ...
    for rec in rec_set:
        role = rec[0]
        cursor.execute("begin")
        cursor.execute("show grants of role " +  crossrep.acctpref_qualifier+role)
        cursor.execute("insert into " + tbname + " select distinct $1 created, $2 role_name, $3 granted_to , $4 grantee_name, $5 granted_by from table(result_scan(last_query_id()))")
        cursor.execute("commit")

    if crossrep.verbose == True:
        print('Finish creating parent_child table ...')
'''

def crParentandPrivilegesAU(cursor): # Renga added new function
    if crossrep.verbose == True:
        print('Starting create parent_child table from acctusage...')
    ...
    au_tbl_list = ["grants_to_roles", "grants_to_users"]
    for t in au_tbl_list:
        cursor.execute("""
...
def crParent( cursor):
    ...
    for rec in rec_set:
        role = rec[0]
        if crossrep.hasSpecial(role):
            role = '"'+role.strip('"') + '"'
        tb_temp =  "TEMP_"+ tbname+"_"+role
...
# cursor: cursor connects to your snowflake account where it creates tables to store metadata
'''
def updParent( role, cursor):
    tbname = crossrep.tb_pcrl
    tb_temp = "TEMP_" + tbname
    cursor.execute("begin ")
    cursor.execute("show grants of role "+  role)

    if crossrep.verbose == True:
        print("show grants of role " + role)
    cursor.execute("create or replace temp table " + tb_temp  +
        " as select distinct $2 role_name, $3 granted_to , $4 grantee_name, $5 granted_by from table(result_scan(last_query_id()))" )
    cursor.execute("commit")
    ...
    mquery = ("merge into " + tbname + " tgt using (" +
        " select role_name, granted_to , grantee_name, granted_by from "+tb_temp + " where role_name = '"+ role + "'" +
        "                 minus " +
        " select role_name, granted_to , grantee_name, granted_by from "+tbname + " where role_name = '"+ role + "'" +
        "  ) as src on tgt.role_name = src.role_name and tgt.granted_to = src.granted_to and tgt.grantee_name = tgt.grantee_name " +
        " and ( tgt.granted_by = src.granted_by or (tgt.granted_by is null and src.granted_by is null)) " +
        " when not matched then insert (role_name, granted_to, grantee_name , granted_by )" +
        "   values (src.role_name, src.granted_to,  src.grantee_name , src.granted_by )")
    if crossrep.verbose == True:
        print('Merging query: ' + mquery)
    cursor.execute(mquery)
    # delete the ones dropped
...
### create table future_grants for privileges granted for future objects in the schema
...
'''
def crFutureGrants(cursor):
    tbname = crossrep.tb_fgrant
    isCreated = False
    tb_temp = "TEMP_" + tbname
    #cursor.execute("select distinct dbname, name from "+ tbschema + " order by dbname, name" )
    if crossrep.mode == 'CUSTOMER':
        query = (" select distinct CATALOG_NAME, SCHEMA_NAME from snowflake.account_usage.SCHEMATA where SCHEMA_OWNER is not null and DELETED is null order by CATALOG_NAME, SCHEMA_NAME ")
    elif crossrep.mode == 'SNOWFLAKE':
    ...
    for row in record:
        ...
        sname = row[1]
        if crossrep.verbose == True:
            print('schema '+dname + '.'+sname)
        if crossrep.isBlank(sname):
            continue

        dname = crossrep.quoteID(dname)
        sname = crossrep.quoteID(sname)

        scname = crossrep.acctpref_qualifier + dname+"."+sname
        if crossrep.verbose == True:
            print('possible quoted schema '+scname)
        cursor.execute("begin")
        ...
        if  isCreated == False :
            cursor.execute(" create or replace temp table " + tb_temp +
            " as select $2 priv, $3 object_type , $4 object_name, $6 grantee_name, max($7) grant_option from table(result_scan(last_query_id())) group by $4,$3,$2,$6 " )
            isCreated = True
        else:
            cursor.execute(" insert into " + tb_temp +
            " select  $2 priv, $3 object_type , $4 object_name, $6 grantee_name, max($7) grant_option from table(result_scan(last_query_id())) group by $4,$3,$2,$6 " )
        cursor.execute("commit")
    ...
    for r in rec:
        if r[0] == 0:
            if crossrep.verbose == True:
                print ('create new future_grants table')
            cursor.execute(" create table " + tbname +
                " as select priv, object_type , object_name, grantee_name, grant_option from " + tb_temp )
        else:
            if crossrep.verbose == True:
                print('update future_grants table')
            updFutureGrants(tb_temp, cursor)

    cursor.execute("drop table if exists " + tb_temp )
    cursor.execute("commit")
    if crossrep.verbose == True:
        print('Finish creating future grants')
'''
def crFGrant(cursor, dblist=None):
    inlist = ''
    if dblist:
        inlist = crossrep.genInList(dblist)
    if inlist != '':
        inPred = "and catalog_name " + inlist
    else :
        inPred = ''
    if crossrep.verbose == True:
        print('Creating future grants ...')
    ...
    if crossrep.mode == 'CUSTOMER':
        query = (" select distinct CATALOG_NAME, SCHEMA_NAME from snowflake.account_usage.SCHEMATA where SCHEMA_OWNER is not null and DELETED is null " + inPred + " order by CATALOG_NAME, SCHEMA_NAME ")
    elif crossrep.mode == 'SNOWFLAKE':
        query = (" select distinct CATALOG_NAME, SCHEMA_NAME from " + crossrep.tb_sc + " where SCHEMA_OWNER is not null order by CATALOG_NAME, SCHEMA_NAME ")
    if crossrep.verbose == True:
        print(query)
...
### Insert future_grants table using current information stored in a temp table
# tb_fgrant : future_grants table storing all future grants information
# tb_temp: temp table storing the current future_grants information (real time)
# cursor: cursor connects to your snowflake account where it creates tables to store metadata
...
### update future_grants table using current information stored in a temp table
### if the current record exists in the table, update it; if not exists in the table, insert it;
### if not exists in current temp table but exists in the future_grants table, delete it
# tb_fgrant : future_grants table storing all future grants information
# tb_temp: temp table storing the current future_grants information (real time)
# cursor: cursor connects to your snowflake account where it creates tables to store metadata
'''
def updFutureGrants(tb_temp,cursor):
    tbname = crossrep.tb_fgrant
    if crossrep.verbose == True:
        print('Start updating future_grants table')
    # insert/update the delta of new information
    mquery = ("merge into " + tbname + " tgt using (" +
        " select priv, object_type , object_name, grantee_name from "+tb_temp +
        "                 minus " +
        " select priv, object_type , object_name, grantee_name from "+tbname +
        "  ) as src on tgt.priv = src.priv and tgt.object_type = src.object_type and tgt.object_name = src.object_name and tgt.grantee_name = src.grantee_name" +
        " when matched then update set tgt.grant_option = src.grant_option" +
        " when not matched then insert (priv, object_type , object_name, grantee_name, grant_option)" +
        "   values (src.priv, src.object_type , src.object_name, src.grantee_name, src.grant_option )")
    ...
    cursor.execute("commit")
    if crossrep.verbose == True:
        print('Finish updating future_grants table')
'''
...
### updating dropped role in PARENT_CHILD and PRIVILEGES tables
